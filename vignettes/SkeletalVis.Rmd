...
```
### Getting started
The package needs to be loaded into a user’s R session.
```{r setup}
library(SkeletalVis)
library(feather)
```
...
First we need to download the gene expression data and metadata.

```{r}
skeletalvis_folder <- download_skeletalvis()
```
The data is stored as a set of feather files, which allow on disk storage of large datsets containing the foldchanges, pvalues and metadata files describing each dataset.
```{r}
skeletalvis <- load_skeletalvis(skeletalvis_folder)
```

## Exploring skeletalvis

We can explore SkeletalVis in multiple ways, finding genes of interet, finding an experimental of interest or finding experiments that have gene expression profile similar to a query.

We can view the differential expression of a set of genes of interest with the `get_gene_data` function.
```{r}
gene_results <- get_gene_data(skeletalvis, c("SOX9","COL2A1"), pvalues=TRUE)
```
### Find a dataset
