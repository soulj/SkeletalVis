...
The `load_skeletalvis` function will check if the `SkeletalVis` data has been downloaded previously and if not ask to download the files. The skeletalvis variable now shows where the data is stored on your system.

```{r eval=FALSE}
skeletalvis <- load_skeletalvis()
```

For this vignette we will work with a small demo version of SkeletalVis to demonstrate the functions.
...
We can compare gene expression profiles against the `SkeletalVis` database using cosine similarity which allows as using the log2 fold changes as a continuous measure of similarity. Normalising the cosine scores to z-scores (how many standard deviations from the mean similarity) allows a assessment of the strength of the similarity compared to the rest of the gene expression profiles. The input dataset should have Human gene symbols and log2 fold changes from all measured genes.
...
To cite OATargets please reference: [10.1136/annrheumdis-2020-218344](https://doi.org/10.1136/annrheumdis-2020-218344)

To cite SkeletalVis please reference: [10.1093/bioinformatics/bty947](https://doi.org/10.1093/bioinformatics/bty947)
