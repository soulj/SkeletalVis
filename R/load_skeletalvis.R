...
#' Find the SkeletalVis data folder and download data if not present
#'
#' Checks the default SkeletalVis data location for the presence of the expected data and metadata files.
#' If these files do not exist, it requests to download the files from the inbuilt URLs.
#'
...
load_skeletalvis <- function(verbose=FALSE, ask=TRUE, demo=FALSE) {
  ...
  urls <- c("https://www.dropbox.com/scl/fi/cslaigrypuw671zn3sirf/accessions.txt?rlkey=x9swp43drswlfqd9uyt3g8elp&st=4023lmdh&dl=1",
            ...
            "https://www.dropbox.com/scl/fi/s8fw2d3ygzho2yrvgx42w/oatargets_prioritised.txt?rlkey=v45j97nh7yhyr9d2e1frb8hit&st=wj20kn1u&dl=1")
